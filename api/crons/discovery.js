...
import { makeApiCallWithCount } from "../utils/apiTracker.js"; // Note the path change
...
export default async function handler(req, res) {
  try {
    ...
    res.status(200).send("Discovery scan completed successfully.");
  } catch (error) {
    console.error(
      "DISCOVERY WORKER: A fatal error occurred during the scheduled run:",
      error
    );
    res.status(500).send("Discovery scan failed.");
  }
}
